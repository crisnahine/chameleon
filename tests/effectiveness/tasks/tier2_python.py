"""Tier-2 (full) Python tasks against CHAMELEON_TEST_PYTHON_REPO.
...
TASKS = [
    EffTask(
        task_id="t2-py-convention-feature-slice",
        tier="full",
        fixture="env-py",
        prompt=(
            "Add a small feature the way this codebase would: a utility that "
            "formats a relative timestamp ('3 minutes ago', '2 days ago') and "
            "one usage of it in an existing module of your choice. Match the "
            "repo's file placement, naming, import style, and export "
    ...
    EffTask(
        task_id="t2-py-crossfile-rename",
        tier="full",
        fixture="env-py",
        prompt=(
    ...
    EffTask(
        task_id="t2-py-duplication-helper",
        tier="full",
        fixture="env-py",
        prompt=(
            "Several places in this repo format money/amounts for display. "
            "Pick one module or view that formats an amount inline and fix "
    ...
    EffTask(
        task_id="t2-py-verification-regression",
        tier="full",
        fixture="env-py",
        prompt=(
...
def _resolve_py_crossfile_target(repo_root: Path) -> dict | None:
    ...
    (not via load_calls_index) so resolution needs no trust state. Leading-
    underscore names are skipped: a dunder or private helper is not an honest
    repo-wide rename target."""
    ...
    for module in sorted(callees):
        ...
        for function in sorted(by_name):
            ...
            if isinstance(body, dict) and isinstance(body.get("total"), int):
                if body["total"] >= 3 and function.isidentifier() and not function.startswith("_"):
                    return {
                        "module": module,
                        "function": function,
                        "new_name": f"{function}_renamed",
                        # Qualified call form for the staleness grep; the
                        # bare name may appear in imports and prose.
...
RUNTIME_TARGET_RESOLVERS = {
    "t2-py-crossfile-rename": _resolve_py_crossfile_target,
}
