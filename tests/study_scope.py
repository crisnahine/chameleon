"""Scope a study measurement to the violations a commit actually INTRODUCED.
...
the same direction, because the level term swamps the delta.
...
def net_new(current: list[dict], baseline: list[dict]) -> tuple[list[dict], int]:
    ...
    that hides it cannot be audited.
    """
    ...
    return introduced, sum(Counter(violation_key(r) for r in baseline).values())
