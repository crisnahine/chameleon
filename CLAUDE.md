...
Go, Rust, Java, C# and PHP are supported at the **extraction tier**: derivation (archetypes, canonical witness, conventions, signatures, imports) but no lint rules, no secret/eval detection, no reverse index, and same-file call edges only. A language there is a declarative spec in `plugin/mcp/chameleon_mcp/extractors/treesitter/lang/specs.py`, not a table module. `plugin/mcp/chameleon_mcp/language_support.py` is the single source of truth for what each tier gets, and its tests hold that declaration to the real wiring — never widen a tier there without widening the wiring, since a rule that never fires reads as a clean codebase.
