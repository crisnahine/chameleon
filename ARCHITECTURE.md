...
> **Status:** v5 after Round 5 10-expert verification (25+ years each). Addresses all 5 NEEDS REVISION findings + critical APPROVED-WITH-NOTES items.
> **Date:** 2026-05-10
> **Author:** Cris Nahine + Claude
> **Review history:** v1 → Round 1 (6 agents) → v2 → Round 2 adversarial (5 agents) → v3 → Round 3 Jesse final → v3 cleanup → Round 4 elite tier (5 agents) → v4 → EF dogfood verification → Round 5 (10 expert agents) → v5
...
- **[TBD]** — claim is provisional, deferred to implementation phase
- **[ASPIRATIONAL]** — claim is a goal, not yet committed

## Table of Contents

- [Purpose](#purpose)
- [Real Problem Evidence](#real-problem-evidence) — CI-gated
- [Goals](#goals)
- [EF Stakeholder Confirmation](#ef-stakeholder-confirmation) — Round 5 add
- [Risk Registry](#risk-registry) — Round 5 add
- [Success Metrics](#success-metrics) — Round 5 add
...
- [**Cluster signature function**](#cluster-signature-function) — Round 5 NEW
- [Profile schema](#profile-schema)
- [**SQLite schemas**](#sqlite-schemas) — Round 5 NEW
- [Atomicity & Crash Safety](#atomicity--crash-safety)
- [**Performance characteristics**](#performance-characteristics) — Round 5 NEW
...
- [**Failure mode runbook**](#failure-mode-runbook) — Round 5 expanded
- [Phase plan](#phase-plan)
- [**License + BC contract**](#license--backwards-compatibility-contract) — Round 5 NEW
- [Open decisions for future iterations](#open-decisions-for-future-iterations)
- [Out of scope for v1](#out-of-scope-for-v1)
- [Inheritance from claude-measure-twice](#inheritance-from-claude-measure-twice)
- [Glossary appendix](#glossary-appendix) — Round 5 NEW

---

## What changed from v4 (Round 5 changelog)

### Critical engineering additions (5 NEEDS REVISION fixes)

40. **[VERIFIED] Cluster signature function specified** (NEW SECTION) — addresses Compiler/Static-Analysis NEEDS REVISION. Defines `f: file → cluster_key` as a 7-tuple. Commits to `ts.createSourceFile` (syntax-only, no type checker). Specifies recompute-all-from-cached-signatures incremental algorithm. Parser-error tolerance: skip files with >20 parse diagnostics.

41. **[VERIFIED] SQLite schemas specified** (NEW SECTION) — addresses Database/Consistency NEEDS REVISION. Full DDL for `drift.db`, `index.db`, `value_attrib.db`. Hashing: xxhash64. `merge_profiles` algorithm: reproposed-profile-from-union with deterministic tie-breaking. Double-fstat loader pattern with generation counter.

42. **[VERIFIED] Performance characteristics + daemonization** (NEW SECTION) — addresses Performance NEEDS REVISION. `ts_dump.mjs` MUST be a long-lived process (per-file invocation = 29 minutes for 5,000 files). MCP server daemon model. Throughput floor: ≥50 files/sec/core. Memory: 100 MB RSS hard cap.

43. **[VERIFIED] Risk registry** (NEW SECTION) — addresses Engineering Manager NEEDS REVISION. 10 prioritized risks with probability × impact × mitigation × owner.

44. **[TBD] EF stakeholder confirmation** (NEW SECTION) — placeholder for documented buy-in from EF engineering manager. Required before Phase 1 begins.

45. **[VERIFIED] Success metrics measurable** — replaces "≥80% pattern conformance" with: "On next 10 EF PRs with AI-generated code, fewer than 2 reviewer comments mention shape/naming/idiom violations chameleon should have caught."

46. **[VERIFIED] License + BC contract** (NEW SECTION) — License declaration + backwards-compatibility commitment.

47. **[VERIFIED] Failure mode runbook expanded** — each failure mode has Detection / Diagnostic / Rollback / Runbook columns.

48. **[VERIFIED] Honest effort estimates** — "10 weeks at solo full-time, 9-15 months at solo nights/weekends; conditional on no scope additions."

49. **[VERIFIED] Glossary appendix** — 40+ terms defined.

50. **[VERIFIED] TOC + certainty markers** — auto-anchored TOC; `[VERIFIED]/[ESTIMATED]/[TBD]/[ASPIRATIONAL]` markers throughout.

### Important APPROVED-WITH-NOTES items addressed

- **`get_pattern_context(file_path)` collapse** (API Designer) — single MCP call replaces 4 round trips per edit.
- **`/chameleon-reset` command** (UX Researcher) — archive + re-init; stops `rm -rf` reflex.
- **Material-change predicate for trust** (UX Researcher) — silent updates vs re-prompt-trigger defined.
- **Provenance line in advisory injections** (UX Researcher) — `[chameleon: archetype=X, confidence=high]`.
- **Auth/crypto archetypes excluded from auto-selection** (AppSec) — v1 limitation documented.
- **Python supply chain integrity** (AppSec) — `pip install --require-hashes`, mirrors TS approach.
- **`<chameleon-context>` doctrine** (AppSec) — "semi-trusted input to non-deterministic interpreter" framing.
- **API versioning envelope** (API Designer) — `{api_version, data, truncated?, next_cursor?}` on all responses.
- **Tool naming alignment** — `refine_profile` MCP renamed to `teach_profile` (matches slash command).
- **Cursor pagination from day 1** — `list_profiles` ships with cursor.

### Engineering Manager's strongest message (acknowledged but proceeding)

> "Architecture has stopped being a design document and become a substitute for shipping... You cannot review your way to perfection."

The author chose to address all NEEDS REVISION findings before Phase 1 implementation. v5 is the result. **Review moratorium declared after v5: implementation findings replace reviewer findings from this point forward.**
...
## EF Stakeholder Confirmation

> **[TBD] This section requires a documented conversation with the EF engineering manager before Phase 1 begins.**

**Status:** Pending. The architecture references "EF api/client as dogfood" 30+ times but no documented buy-in exists from the EF engineering team.

**Required before Phase 1 starts:**
- Conversation with EF engineering manager: "If chameleon worked, would you adopt it on api or client? Who'd champion it? What's the bar?"
- Named champion at EF documented here
- Adoption commitment level (mandatory tool / opt-in tool / experimental)
- Slot on team roadmap (if relevant)
- Bar for "good enough" from EF's reviewer perspective (Round 5 success metrics below)

**If EF declines to adopt:**
- The Real Problem Evidence CI gate (5+ transcripts from EF dogfooding) becomes unfillable
- v1.0 cannot ship as currently designed
- Project must restructure: alternative dogfood target, or accept indefinite "v0.x experimental" status

**Owner:** Cris. **Deadline:** before Phase 1 starts (else Phase 1 cannot start).

---

## Risk Registry

10 prioritized risks for Phase 1+. Mitigations either documented elsewhere in this architecture or flagged as TBD.

| # | Risk | Probability | Impact | Mitigation | Owner |
|---|---|---|---|---|---|
| 1 | EF declines adoption; Real Problem Evidence unfillable | Medium | Critical (v1.0 cannot ship) | EF stakeholder conversation BEFORE Phase 1 | Cris |
| 2 | TS Compiler API subprocess overhead blows 5s/file budget on real EF client | High | High (Phase 2 grinds) | Daemonize ts_dump.mjs (see Performance section) | Cris |
| 3 | AST clustering produces low-confidence output on real EF code | High | Critical (80% conformance gate fails) | Run early on EF client subset; iterate signature function | Cris |
...
| 7 | EF buy-in obtained but team uses CLAUDE.md instead | Low | High (no signal for value) | Frame chameleon as CLAUDE.md complement, not replacement | Cris |
| 8 | Profile.json merge conflicts cause team pain | Medium | Medium | merge_profiles tool + .gitattributes template (designed) | Cris |
...
> **On the next 10 EF api/client PRs that include AI-generated code (with chameleon active), fewer than 2 reviewer comments mention shape/naming/idiom violations that chameleon should have caught.**
...
> **[VERIFIED] Round 5 NEW** — addresses Compiler/Static-Analysis NEEDS REVISION verdict.
...
> **[VERIFIED] Round 5 NEW** — addresses Database/Consistency NEEDS REVISION verdict.
...
The atomic-rename pattern (Round 4) gives write-side cross-file atomicity. **Reader-side** consistency requires the double-fstat pattern:
...
> **[VERIFIED] Round 5 NEW** — addresses Performance NEEDS REVISION verdict.
...
- CI benchmark on EF dogfood corpus + 3 OSS TS repos
...
> **[VERIFIED] Round 5 NEW** — addresses OSS Maintainer NEEDS REVISION verdict.

### License declaration

**Status:** [TBD] — author choice required before any public release.

**Recommendation:**
- If chameleon stays private to EF: `UNLICENSED — proprietary to Empire Flippers, LLC` (matches predecessor claude-measure-twice)
...
[TBD] — author choice. Recommendations:
...
If staying private to EF, internal IP assignment via employment contract suffices.

---

## Glossary appendix

> **[VERIFIED] Round 5 NEW** — addresses Technical Writer NEEDS REVISION verdict.
...
| **`<chameleon-context>`** | Neutral XML-style tag wrapping injected context. NOT framed with importance ("EXTREMELY_IMPORTANT") to avoid framing competition with superpowers. |
...
*End of v5 architecture. Addresses all 5 NEEDS REVISION findings + critical APPROVED-WITH-NOTES items from Round 5 10-expert verification.*

**Total review investment:**
- 5 rounds of multi-agent review (Round 1: 6 agents, Round 2: 5, Round 3: 1, Round 4: 5, Round 5: 10) = 27 unique reviewer perspectives
- EF dogfood verification against /api and /client repos
- v1 (3,899 words) → v5 (~12,000+ words)

**Review moratorium declared after v5.** Implementation findings replace reviewer findings from this point forward. The Engineering Manager's voice from Round 5 was clear: "You cannot review your way to perfection." Real learning starts with code.

Phase 1 prerequisites:
1. **EF stakeholder confirmation conversation** — gates everything else
