...
def _sync_rules_import(profile_dir: Path) -> None:
    ...
    try:
        ...
        if rules_path.is_file():
            try:
                # Repo-resident and teammate-controllable, so it goes through the
                # containment/size/symlink-checked reader like every other repo
                # read here -- a committed symlink to a FIFO or a huge file would
                # otherwise hang or blow memory inside a teach.
                existing = safe_read_text(repo_root, rules_rel, max_size_bytes=64_000)
            except Exception:
