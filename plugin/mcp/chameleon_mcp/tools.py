...
_RULES_IMPORT_BODY = f"{_RULES_IMPORT_MARKER}\n@../../.chameleon/conventions.md\n"
...
def _sync_rules_import(profile_dir: Path) -> None:
    ...
    injected by a hook. Both the mirror's own header and the init skill already
    name `.claude/rules/chameleon-conventions.md` as the way to wire it, but
    creating it depended on the model offering the step and the user accepting
    it -- so the highest-authority channel chameleon has was, in practice,
    wired by prose and luck. An unconditioned rule file (no ``paths`` frontmatter)
    loads at launch at CLAUDE.md priority, which is what makes this the reliable
    ...
    Never touches a file it did not author: an existing file without the marker
    is left exactly as-is, the same posture the statusline wiring takes toward a
    statusLine the user already configured. A marked file is rewritten only when
    its content drifted, and removed when the mirror itself is gone.
    """
    if os.environ.get("CHAMELEON_CONVENTIONS_MD") == "0":
        return
    try:
        repo_root = profile_dir.parent
        rules_path = repo_root / ".claude" / "rules" / "chameleon-conventions.md"
        ...
        if rules_path.is_file():
            try:
                ...
                # Unreadable but present: it is not ours to judge, let alone
                # replace. Leaving it is the only safe read of an ambiguous file.
                return
...
def _sync_conventions_md(profile_dir: Path, conv: dict) -> None:
    ...
    The import that makes the mirror load is synced in the same call, so the two
    cannot drift: a mirror nothing imports is inert, and an import pointing at a
    removed mirror is worse than neither."""
    if os.environ.get("CHAMELEON_CONVENTIONS_MD") == "0":
        return
    _write_conventions_mirror(profile_dir, conv)
    _sync_rules_import(profile_dir)


def _write_conventions_mirror(profile_dir: Path, conv: dict) -> None:
    """Render and place `.chameleon/conventions.md`; see `_sync_conventions_md`."""
    try:
