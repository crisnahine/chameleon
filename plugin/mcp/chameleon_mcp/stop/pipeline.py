...
slice of a Stop invocation) and ``stop_gates`` (the per-root gate pipeline
``_gate_one_root`` calls once per discovered workspace).
...
drift). It never emits; the caller (``_gate_one_root`` / ``stop_backstop``) is
the sole place a hook-output dict reaches stdout.

Extracted verbatim from ``hook_helper.py``'s ``_stop_gates`` (a ~700-line
function whose call-sites are frozen -- see the ``_stop_gates`` shim in
``hook_helper.py``). Symbols still owned by ``hook_helper`` (the judge/
multi-lens/duplication spawn machinery, and every other hook_helper-resident
...
single deferred ``from chameleon_mcp import hook_helper as hh`` import inside
``stop_gates``, so a test that patches ``chameleon_mcp.hook_helper.<name>``
stays effective for a call made from this module -- and so this module's own
