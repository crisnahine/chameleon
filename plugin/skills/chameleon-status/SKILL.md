...
5. **Version coherence** — call `chameleon_telemetry(action="daemon_status")` to get `running_version` (also returns `alive`, `pid`, `socket`, `uptime_s`, `last_request_at`). If the running version differs from the installed plugin version, surface "Running v<X>, installed v<Y> — restart Claude Code to pick up the new MCP."
...
7. **Enforcement** — call `chameleon-mcp::chameleon_telemetry(action="get_status", params={"repo": <repo>})` and surface its `enforcement` block:
   ...
   - `idiom_review` — default on (advisory, never blocks). Gates the idiom lens of the async turn-end review job: findings cite a taught idiom's slug and the offending diff lines, delivered next turn (or in-turn under `CHAMELEON_JUDGE_WAIT`), never a Stop-hook interrupt.
   - `idiom_judge` — vestigial; no longer read by any lens.
   - `multi_lens_review` — vestigial; no longer read by anything. It used to select between a single-lens and a multi-lens turn-end review; every lens now runs from the `stop.lenses.LENSES` registry regardless of this key.
   - `correctness_judge` — default on (advisory, never blocks). Gates the correctness lens of the same async job: an independent reviewer model reads the turn's diffs for correctness bugs; findings arrive as a context block to verify against the code.
...
10. **Shelved findings** - review findings chameleon noticed but did not deliver. Call `chameleon-mcp::chameleon_telemetry(action="get_shelved_findings", params={"repo": <repo>})` and surface `count` plus a handful of `findings` rows (`severity`, `claim`, `file`, `recurrence`). Honesty: shelved means the finding scored below this repo's `review.surface_bar` at write time, not that a human reviewed and dismissed it. Nothing here was delivered, and a shelved row auto-promotes to a real pending finding once it recurs `SHELVED_PROMOTE_MIN_RECURRENCE` times, so print `recurrence` next to each row - a shelved finding with high recurrence is one turn away from surfacing on its own. Point at `/chameleon-explain <match_key>` for a single row's detail.
...
Call `chameleon-mcp::chameleon_telemetry(action="get_shadow_report", params={"repo": <repo>, "window_days": <window_days>})`. `window_days` is optional (default `CHAMELEON_SHADOW_REPORT_WINDOW_DAYS`, 21). Surface its block:
...
- **`idiom_review`** — a turn-level would-block counter, always 0 as of v3.4.0: it counted the once-per-session Stop idiom/principle self-review interrupt (hook `stop-idiom-review`), which no longer exists (idiom review is now an advisory-only lens of the async turn-end review job — see `docs/architecture.md`'s "Turn-end advisories"). Kept in the report shape rather than removed outright; do not read a nonzero value here as evidence of anything on a v3.4.0+ repo.
...
A lead watching a rollout asks "is the AI code staying healthy without humans?" and historically had one trailing number, the drift score, to answer it. That number measures structural mimicry, not correctness, so reading it as a health bar over-trusts it. Call `chameleon-mcp::chameleon_telemetry(action="get_longitudinal_signals", params={"repo": <repo>, "window_days": <window_days>})` (default `CHAMELEON_LONGITUDINAL_WINDOW_DAYS`, 21) and present the two tracks it returns — but **print the `disclaimer` line first, above both tracks**, so neither track is read as a correctness guarantee:
