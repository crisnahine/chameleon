...
- **Idiom review** — `enforcement.idiom_review` (default on). When a turn edited files governed by team idioms/principles and no lint block fired, the Stop hook blocks ONCE per session to make you self-review those changes against the idioms (`idioms.md`) and principles (`principles.md`). Fix any clear violation, then end again to confirm. To skip the check, add `// chameleon-ignore idioms` (`# chameleon-ignore idioms` in Ruby) in a file you touched. `enforcement.idiom_judge` (default on) strengthens the directive to demand a thorough review; it does not yet spawn an independent judge.
