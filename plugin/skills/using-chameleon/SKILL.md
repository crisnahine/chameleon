...
- **Planning, or dispatching an implementer.** Put the archetype and its canonical witness path into the plan's Files block or the dispatch brief BEFORE the implementer writes. (A witness is a `path`, not a `file:line` - the record carries no line number.) `superpowers:writing-plans` and `superpowers:subagent-driven-development` both say "follow established patterns" without any way to know what they are; the PreToolUse advisory only arrives after the implementer has already chosen a shape. Under subagent-driven development the brief is by contract the implementer's *only* source - it is told never to read the plan file - so a fact that is not in the brief does not reach it at all.
