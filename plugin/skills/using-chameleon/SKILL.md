...
- **Turn-end review (async-first)** — at Stop, chameleon's deterministic gates decide whether the turn warrants a model review and, if so, launch ONE detached background job; the Stop hook itself never waits on it (never blocks, never delays turn end). The job runs up to three lenses over the turn's diffs — **correctness** (`enforcement.correctness_judge`, default on: inverted conditions, missing guards, dropped awaits, other logic bugs), **duplication** (`enforcement.duplication_review`, default on: a new function that re-implements one already in the committed catalog or added earlier this session), and **idiom** (`enforcement.idiom_review`, default on: a diff hunk that violates a taught team idiom, citing the idiom's slug and the offending lines) — then independently VERIFIES each finding with an independent refuter before it can surface (a refuted finding is dropped; survivors are tagged `[confirmed]` or `[unverified]`). Findings deliver as a `[🦎 ...]` context block at your NEXT prompt (or SessionStart if the session ended first) — never in the same turn that triggered the review, unless the harness/CI flag `CHAMELEON_JUDGE_WAIT=1` is set, in which case Stop waits for the job and renders in-turn. A finding whose cited file changed since review renders `[stale]` rather than being silently dropped. Treat every finding as a lead from an independent reviewer: verify it against the code before acting — findings may be wrong, but do not dismiss them unread. The whole pipeline fails open (no `claude` CLI, timeout, disabled → no findings, never a delay). Set `enforcement.idiom_review: false` / `enforcement.duplication_review: false` / `enforcement.correctness_judge: false` in `.chameleon/config.json` to durably drop one lens; a compliant turn shows nothing at all — there is no more once-per-session self-review interrupt to dismiss.
...
- `enforce` — default; real deny/block on rules calibration kept active for this repo, plus deterministic security facts (hard-kind credentials, `eval`/`exec`).
...
When superpowers is also installed, composition runs on two surfaces. SessionStart appends a compressed routing paragraph to the operational digest, and invoking any superpowers skill injects that skill's own contract at the call - a PreToolUse hook on the `Skill` tool for a skill the model chooses, and the UserPromptSubmit hook for one the user types as `/superpowers:<name>`, which is expanded inline and emits no tool call at all. This section is the full version of the contract; the digest is its summary, not a second source of truth.

Detection differs by surface, and the per-skill one is the stronger of the two. The digest block asks `peer_plugins.superpowers_installed` whether superpowers WILL be active - existence checks only, never reading the peer plugin's content. The per-skill block needs no detection: `superpowers:<name>` arriving from the Skill tool is itself proof the plugin is loaded. `CHAMELEON_PEER_ROUTING=0` disables both, and any detection error reads as "not installed", so chameleon behaves exactly as it does alone.
...
- **Planning, or dispatching an implementer.** Put the archetype and its canonical witness path into the plan's Files block or the dispatch brief BEFORE the implementer writes. (A witness is a `path`, not a `file:line` - the record carries no line number.) `superpowers:writing-plans`, `superpowers:subagent-driven-development` and `superpowers:dispatching-parallel-agents` all say "follow established patterns" - or require a self-contained brief - without any way to know what those patterns are; the PreToolUse advisory only arrives after the implementer has already chosen a shape. Under subagent-driven development the brief is by contract the implementer's *only* source - it is told never to read the plan file - so a fact that is not in the brief does not reach it at all.
- **Executing a plan someone else wrote.** The plan's facts are as old as the plan. When its named archetype or witness contradicts the `<chameleon-context>` block that arrives at the edit, the live block wins, and the disagreement is worth surfacing - it means the plan predates a refresh, a merge, or another checkout.
- **Requesting review.** `/chameleon-pr-review` is the stronger reviewer - it inlines that reviewer's discipline and adds grounded cross-file facts - but it is NOT a superset, so say "prefer", not "supersedes". Two things it does not carry: `superpowers:requesting-code-review` exists to dispatch a reviewer SUBAGENT so the diff never enters the coordinator's window, while pr-review runs its convention and logic passes in the coordinator's own context unless the engine recommends fan-out; and that reviewer's "are all tests passing?" verdict input is out of scope for a static review that runs nothing. Keep the superpowers skill for the final whole-branch review of a subagent-driven run, which is dispatched with a review-package path and a parked-findings list - a pair pr-review does not take. BLOCK/FIX/NIT map to Critical/Important/Minor.
- **Receiving review.** `/chameleon-receiving-code-review` supersedes `superpowers:receiving-code-review`, and this one IS a genuine superset: it inlines that discipline, then verifies each comment against the code and the repo's conventions before apply-or-push-back, applies fixes one at a time after per-item approval, and drafts replies without ever posting them.
- **Debugging.** `superpowers:systematic-debugging` owns the sequence. Chameleon serves two steps inside it, not the whole of Phase 1: step 5's backward trace from a bad value to its origin is `get_callers` / `get_blast_radius` work (`get_callees` is the forward counterpart, for what a function calls), and Phase 2's "find a working example" is `search_codebase` / `get_canonical_excerpt`. Steps 1-4 - read the error, reproduce, check recent changes, instrument boundaries - are outside anything chameleon knows. Use the tools inside the process, not instead of it.
- **Writing the first failing test.** `superpowers:test-driven-development` decides WHETHER the test gets written, and its Iron Law wins. Chameleon's derived "match sibling test shape; skip tests where siblings have none" describes what the repo currently does, which is not a licence to skip a test the user asked for first; it governs the test's SHAPE only.
...
- **Subagent edits are already guarded.** PreToolUse and PostToolUse fire per tool call, so a superpowers implementer subagent gets archetype context and linting on every edit. The turn-end review job never spawns per subagent (top-level Stop only), so an SDD run pays for it once. Each dispatched agent also gets its OWN first-in-archetype: the "already shown this archetype" set is keyed per agent, because a fresh implementer starts with an empty window even though it shares the coordinator's session id.
