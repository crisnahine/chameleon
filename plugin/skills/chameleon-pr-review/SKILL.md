...
- The cross-file tools (`get_duplication_candidates`, `get_crossfile_context`, `get_callers`, `query_symbol_importers`) read prebuilt profile artifacts; they make no network call and run no repo code. Do not relay a duplication finding without a returned candidate, or an existence break without `high_confidence`, and never read an empty `get_callers`/`query_symbol_importers` result as dead code — `query_symbol_importers` catches a barrel/index-re-exported symbol `get_callers` cannot see (it tracks only direct call expressions), but its absence is no more conclusive than an empty `get_callers`.
