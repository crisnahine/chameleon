...
Call `chameleon_review(action="get_review_history", params={"repo": <repo_id>, "limit": 5})` once. The records are `data.records` (a list; `data.total` is the RECORD count, not a finding count), each carrying `commit_sha`, `verdict`, `complexity_tier`, `pr_id`, a `verified` flag, and `findings` — and `findings` is a `{severity: count}` DICT, not an integer, so N is a value you sum or read out of it, never the field itself (printing `record["findings"]` renders `{'total': 5}`). Skip any record whose `verified` is false: the ledger is tamper-evident, and an unverified row is exactly the one not to trust. If a verified record pins
...
It returns `{auto_pass_eligible, risk, complexity_tier, reasons, facts, changed_files}` plus `advisory`, `base_ref`, `contract_breaks`, `typecheck`, `tests`, and `fan_out`. `contract_breaks` is the SAME defect class Step 2.9e reports from `get_contract_breaks` — both surfaces call one helper behind one cap and one trust gate, so this is not an independent second opinion and never a backstop for a degraded 2.9e: whatever empties 2.9e (diff too large, missing calls index, untrusted profile) empties this list too. It is CONTEXT here and never a finding: One citation per break, and it is Step 2.9e's, in 2.9e's shape. If this envelope carries breaks that 2.9e did not report, the discrepancy is a signal to re-run 2.9e, not licence to emit a FIX from a step that emits none. Report the rest verbatim as an advisory line (the Auto-pass routing section in Step 4). It is ADVISORY only: it never produces a BLOCK, FIX, or NIT and never changes the verdict. Its job is to mark the safe-to-skip slice, grade the change's inherent complexity (`complexity_tier`: easy / medium / hard / complex — structural, independent of cleanliness), and name why a change is NOT in the skip slice (a security-sensitive surface, too large, high cross-file blast radius, a file outside the profiled archetypes, or a grounded block finding).
