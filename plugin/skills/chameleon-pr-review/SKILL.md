...
It returns `{auto_pass_eligible, risk, complexity_tier, reasons, facts, changed_files}` plus `advisory`, `base_ref`, `contract_breaks`, `typecheck`, `tests`, and `fan_out`. `contract_breaks` is the SAME defect class Step 2.9e reports from `get_contract_breaks`: it is context here, never a second finding — when 2.9e already cited a narrowing, do not cite it again from this envelope, and when 2.9e was skipped or degraded, cite it once with 2.9e's citation shape. One citation per break, whichever call surfaced it. Report the rest verbatim as an advisory line (the Auto-pass routing section in Step 4). It is ADVISORY only: it never produces a BLOCK, FIX, or NIT and never changes the verdict. Its job is to mark the safe-to-skip slice, grade the change's inherent complexity (`complexity_tier`: easy / medium / hard / complex — structural, independent of cleanliness), and name why a change is NOT in the skip slice (a security-sensitive surface, too large, high cross-file blast radius, a file outside the profiled archetypes, or a grounded block finding).
