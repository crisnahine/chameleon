...
Run this whenever the diff touches a dependency manifest or lockfile of ANY ecosystem — the npm/Bundler set the tool parses (`package.json`, `package-lock.json`, `npm-shrinkwrap.json`, `yarn.lock`, `pnpm-lock.yaml`, `Gemfile`, `Gemfile.lock`, and `*.gemspec` — a gem declares its runtime dependencies there via `add_dependency`, not a Gemfile) AND the ones it does not (`setup.py`, which is arbitrary code, and the foreign lockfiles). The parsed set now spans Python (`requirements*.txt`, `pyproject.toml`, `Pipfile`, `setup.cfg`), Go (`go.mod`), Rust (`Cargo.toml`), PHP (`composer.json`) and Java (`pom.xml`, `build.gradle`, `build.gradle.kts`). Always call `scan_dependency_changes` (below) — it parses those and reports the remainder in `uncovered_manifests` for the hand-review disclosure. These are the supply-chain entry points a human reviewer reads line by line and the convention review above does not cover. This pass is a pure parse of the diff text and the manifest/lockfile JSON or YAML. It makes NO network calls and does not install or run anything: only the added (`+`) lines matter, and the existing repo content gives the "previously present" baseline.
...
**Uncovered manifests — disclose AND hand-review, split by severity exactly as the parsed path does.** The scanner reads npm, Bundler, Python, Go, Rust, PHP and Java manifests. What it still does not parse is `setup.py` (arbitrary code) and the foreign lockfiles (`poetry.lock`, `Pipfile.lock`, `go.sum`, `Cargo.lock`, `composer.lock`), which carry no resolved-host keyword to key on. A changed file of that kind comes back in the envelope's `uncovered_manifests` list (with `findings` empty and `manifests_changed` empty, which for those files means "not parsed", NOT "reviewed clean"). When `uncovered_manifests` is non-empty, the tool could not scan it, but YOU can still read the added (`+`) lines — so hand-review them and route each signal to the SAME severity the npm/Bundler checks give the identical content. One limitation applies to a PARSED manifest too: a hunk whose section opener sits outside the diff's context window (a composer `require` block, a long poetry table) yields nothing, and nothing is not the same as clean. When a manifest you know changed produced no finding, read its added lines yourself before calling it clean.

Do not lump everything into an ACK: a blatant supply-chain red flag that is plainly visible in the diff is a witnessed finding, and suppressing it because the parser was silent inverts the very asymmetry the ACK rule exists to prevent.
