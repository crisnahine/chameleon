...
- **`coverage-gap`** — no archetype matched, or it matched at `match_quality` `none`/`fallback`, AND nothing fired. The per-edit lint never had a calibrated SHAPE to check against, so no shape-specific rule could fire for this file's kind, and the archetype-independent scans (secrets, `eval`, cross-cutting advisories) found nothing either — `decision.violations_raised` is 0 on a coverage-gap row. A raised violation on an uncovered shape classifies as `advised`, not here. Say "no calibrated shape for this file's kind, and no cross-cutting rule fired" rather than "nothing could have been caught". **Route:** `/chameleon-refresh` to re-derive archetypes (the file's directory may have grown into its own archetype since the last bootstrap), or `/chameleon-teach` to capture the missing convention. This is a hole in shape coverage, not a rule failure.
...
- **`advised`** — chameleon *did* raise advisories (or shadow-logged a would-block), but did not block. This outranks `coverage-gap`, so read `decision.match_quality` before routing; it splits the bullet in two.
  - `ast`/`exact` — the shape was covered and the shape rules fired; they were advisory, so the edit shipped with the warning attached rather than silently. **Route:** not a coverage hole and not a missing rule, so do not `/chameleon-refresh` (it changes nothing here). Ask whether the advisory was surfaced and heeded, and whether this defect class warrants promoting a rule to block-eligible — read `decision.blockable_rules` and the rule's override history via the `/chameleon-explain <rule>` flow above.
  - `none`/`fallback` (or `null`) — the shape was NOT covered; what fired was an archetype-independent scan (a secret, an `eval`), which runs regardless of archetype. **Route:** both questions apply. Ask the rule-strength question above, AND treat the uncovered shape as a real gap — `/chameleon-refresh` or `/chameleon-teach`, exactly as for a `coverage-gap` row.
