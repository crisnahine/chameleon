...
> **Status:** v3 after Round 2 adversarial review (5 agents) + scope simplification. Addresses 9 BLOCKING items + ~25 SIGNIFICANT items. **Two major scope shifts**: (1) drops "framework support" framing entirely — chameleon does best-effort pattern clustering on whatever AST can parse, with idioms.md absorbing what AST cannot; (2) **drops companion plugin pattern entirely from v1** — engine is the only distributed artifact; profile sharing happens via committed `.chameleon/profile.json` in each repo (git is the distribution mechanism).
> **Date:** 2026-05-10
> **Author:** Cris Nahine + Claude
> **Predecessors:** `claude-measure-twice` (EF-specific) — its profiles will be regenerated via `/chameleon-init` on EF api/client repos as the dogfood case; **NOT** distributed as a separate plugin.
> **Versions:** v1 archived at `ARCHITECTURE-v1.md`. v2 archived at `ARCHITECTURE-v2.md`. v3 draft (with companion plugin pattern) archived at `ARCHITECTURE-v3-draft.md`.

---

## What changed from v2 (Round 2 changelog)

### Strategic scope shifts (load-bearing)

**Shift 1: Best-effort, not framework-aware**

v2 claimed: "works on any TypeScript/JavaScript repo in v1; expand to Ruby/Python in v1.5"

v3 says: chameleon does **best-effort pattern clustering** on whatever AST can parse. Where AST + statistics produce clean archetypes (file shape, naming, imports, structural conventions): excellent. Where they don't (NestJS decorators, tRPC builder chains, type-level discriminants, runtime-driven semantics): the engine falls back to interview + `/chameleon-refine` to capture the un-inferable. **Chameleon learns YOUR repo's patterns, not TYPES of repos.**
...
**Shift 2: No companion plugin pattern in v1 (engine is the only artifact)**

v3-draft proposed companion plugins (`chameleon-ef-pack`) as the distribution model for hand-curated profiles.

v3-final drops this entirely for v1. Reasoning:
- Adds significant attack surface (pack signing, version compat, signature verification, supply chain)
- The original goal — "EF api/client get the gold-standard profile" — is achieved by simply running `/chameleon-init` on EF repos and committing the resulting `.chameleon/profile.json` to each repo
- Profile sharing happens via git (the repo IS the distribution mechanism)
- New devs cloning the repo get the profile automatically
- One artifact to maintain (the engine), one workflow to teach (run `/chameleon-init`)
- Companion plugins remain a *future possibility* for v2.0+ if community demand emerges, but explicitly out-of-scope for v1

**EF strategy:** EF api and EF client are the **primary dogfood test cases**, not special. Run `/chameleon-init` on each → commit `.chameleon/profile.json` → done. The Real Problem Evidence transcripts will come from EF dogfooding.
...
3. ❌ Companion plugin pattern with engine version contracts → ✅ **No companion plugins in v1** (entire pack distribution model removed; profiles ship via git per repo)
...
5. **Profile sharing via git** — committed `.chameleon/profile.json` in each repo IS the distribution mechanism (no separate plugin needed)
...
│                                                                          │
│  EF dogfood: run /chameleon-init on EF api + EF client repos →           │
│              commit .chameleon/profile.json to each                       │
│  New EF dev clones repo → profile already there → /chameleon-trust       │
│  → starts editing with full pattern conformance                          │
│                                                                          │
│  No companion plugins in v1. No pack signing infrastructure.             │
│  No --allow-unsigned flag. No version compat between engine and packs.   │
│                                                                          │
│  Companion plugins remain as a v2.0+ possibility if community demand     │
│  emerges, but explicitly out-of-scope for v1.                            │
└──────────────────────────────────────────────────────────────────────────┘
```
...
**Note:** No `packs/` directory exists in v1. EF profiles are NOT distributed as a separate plugin — they live directly in EF api and EF client repos as committed `.chameleon/` directories (the dogfood test case).
...
- Available slash commands (4 user-facing + `/chameleon-trust`)
...
### User-invokable skills (4 commands + 1 trust)
...
### Skill test plans for chameleon-init, refresh, refine, status, trust
...
2. `.chameleon/profile.json` (and friends) are written to the repo and committed
3. Team member opens a PR with the profile as the change
4. Other team members review the PR (using `profile.summary.md` for human-readable diff)
5. Once merged, every dev who pulls the branch gets the profile automatically
6. New devs cloning the repo for the first time → profile is already there
7. Each user runs `/chameleon-trust` once per repo (per-user, non-blocking)

**The repo IS the distribution mechanism.** Git is the package manager.

**EF dogfood case:**
- Run `/chameleon-init` on `empire-flippers/api` repo → commit `.chameleon/` → done
- Run `/chameleon-init` on `empire-flippers/client` repo → commit `.chameleon/` → done
- All EF devs working in those repos get the profile automatically via git pull
- Real Problem Evidence transcripts collected from EF dogfooding feed back into `idioms.md` via `/chameleon-refine`
- claude-measure-twice's hand-curated knowledge informs the initial bootstrap interview answers + idioms collection
- **No `chameleon-ef-pack` separate plugin to maintain**

**What was simplified out (vs v3-draft):**
- ❌ Pack signing (Sigstore/cosign/ed25519) — eliminated, not needed
- ❌ `--allow-unsigned` flag — eliminated
- ❌ Pack version compat (`engine_min_version`/`engine_max_version`) — engine version migrations only matter for the schema in `.chameleon/profile.json`, which is committed and versioned with the repo
- ❌ Pack ID namespace (`<publisher>/<pack>`) — eliminated
- ❌ Pack supply chain attack surface — eliminated
- ❌ "Max companion packs per session" guard — eliminated
- ❌ Pack abandonment policy — eliminated

**What remains:**
- ✅ Profile schema versioning (engine vN supports schemas v(N-1) to v(N))
- ✅ Per-repo trust model (committed profile = data; user must trust per-repo)
- ✅ Profile schema validation on load
...
If, after v1 ships and is in real use, community demand emerges for distributing pre-built profiles outside individual repos (e.g., a community-maintained Rails 7 baseline profile, or a Next.js 14 + Tailwind starter profile), companion plugin distribution can be added as a **non-breaking v2.0 feature**. The engine architecture supports this addition without requiring breaking changes:

- The companion plugin would deliver pack files to `${PLUGIN_DATA}/packs/<pack-id>/`
- The MCP server gets back the `apply_profile_pack` tool (currently removed)
- Pack signing infrastructure gets added at that point (with the lessons learned from v1)
- Existing `.chameleon/profile.json` workflows continue unchanged
...
2. **Symlink lstat in MCP file reads** — closes TOCTOU exfiltration vector
3. **Hardlink defense** — inode-based dedup catches hardlinks (lstat only refuses S_ISLNK)
4. **HMAC bug fix** — `${TMPDIR:-/tmp}/.chameleon_exec_log/` consistently in both write and read paths
5. **profile.json schema validation** — strict JSON schema; rejects malformed

### Important mitigations
6. **Repo size guard** — 50k file ceiling, post-glob enforced
7. **AST extractor subprocess limits** — 5s CPU + 512 MB RSS + 1 MB file ceiling
8. **Bootstrap interview output sanitization** — strip ANSI/zero-width, 50 KB cap on idioms.md
9. **drift.db local-only** — never committed; in `${PLUGIN_DATA}` not `<repo>`
10. **HMAC key fail-loud** — explicit error if `/dev/urandom` fails
11. **Trust model** — committed profiles untrusted-by-default per user; `/chameleon-trust` to approve

(v3-draft items 7, 8, 15, 16 — pack signing, --allow-unsigned, etc. — REMOVED with companion plugin pattern.)
...
| Phase 3 — Skills with eval | ~60h | All 6 skills (using-chameleon foundation + 5 user-invokable: init, refresh, status, refine, trust) pass RED-GREEN-REFACTOR cycle. Acceptance test transcripts captured (with superpowers active). CI enforcement live. |
| Phase 4 — Security mitigations | ~30h | All 11 mitigations integrated. Schema validation. HMAC bug fix verified. Trust model working. |
| Phase 5 — EF dogfood | ~30h | `/chameleon-init` run on EF api + EF client repos; profiles committed; idioms iterated via `/chameleon-refine`. Trust-on-first-use prompt. **Real Problem Evidence transcripts collected from EF dogfooding.** (Down from v3-draft's 50h — no pack distribution to build.) |
...
| **Total** | **~350h** | **~9 weeks of focused work** (up from v2's 340h due to maintenance scaffolding; down from v3-draft's 370h due to companion plugin removal) |
...
1. **MCP transport beyond stdio** — only if a future platform requires it. v1 is stdio-only.
...
3. **Skill priority with superpowers** — `using-chameleon` documents "after process, before implementation" — codify this in superpowers' priority hierarchy too?
4. **Profile schema v3 → v4 migration** — the migration template exists in `migrations/`; first real migration's complexity is unknown until needed.
5. **Companion plugin pattern (v2.0+)** — if community demand emerges, add pack distribution as non-breaking v2.0 feature. Out of scope for v1.
...
- **Companion plugin / profile pack distribution** (deferred to v2.0+ pending community demand)
...
- Security mitigations (11 items, down from v3-draft's 16 — pack-related items removed)
...
- **Companion plugin pattern in v1** (deferred to v2.0+ pending demand)
- **Pack signing infrastructure** (eliminated with companion plugins)
