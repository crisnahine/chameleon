...
**Escape hatch:** a blocked edit is overridable inline with `// chameleon-ignore <rule>` (`# chameleon-ignore <rule>` in Ruby), or a bare `// chameleon-ignore` to suppress all chameleon blocks on that line. Place the directive ON the offending line, or alone on the line directly above it; `// chameleon-ignore-file <rule>` suppresses the rule for the whole file. The directive must end its line (trailing prose deactivates it) and directives inside string literals do not count. `CHAMELEON_ENFORCE=0` disables all blocking for the session regardless of mode.
