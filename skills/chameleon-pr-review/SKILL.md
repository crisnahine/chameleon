...
#### 2.6a. Secret escalation (BLOCK)

Read the `secret-detected-in-content` violations from each file's Step 2b `lint_file` response. These come from a deterministic secret scan that runs before the archetype match and before the trust gate, so they are present for every file you linted, trusted or not.

Escalate every `secret-detected-in-content` violation to **BLOCK**. A credential committed to the diff is the one security finding here that is a witnessed fact, not a judgment: the scanner matched it in the file content. Cite the file and the line/position the violation reports, and carry the violation's own message (it names the secret kind and tells the author to rotate it).

The secret scan has a known false-positive tail: test fixtures with fake keys, example credentials in docs, UUIDs and high-entropy hashes that are not secrets, and base64 blobs that are data, not keys. Say so on each secret BLOCK so the author can override a genuine fixture. A real fixture key is overridden by the author, not by this review silently dropping the finding: report the BLOCK and label it "verify this is not a live credential; if it is a test fixture, it is safe to keep."
...
Authz and taint/SSRF/traversal findings are capped at FIX. They are advisory judgments and never escalate to BLOCK; only a secret detection blocks from the security pass.
...
- **BLOCK**: any BLOCK finding → verdict is BLOCK. A secret detected in the diff (Step 2.6a) is a BLOCK and drives a BLOCK verdict; an irreversible op in a `change` block (Step 2.7a) is a BLOCK and drives a BLOCK verdict; the advisory authz/taint findings and the migration table-size advisories (Step 2.7b/2.7c) are capped at FIX and never force a BLOCK verdict on their own.
