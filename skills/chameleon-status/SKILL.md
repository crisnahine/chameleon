...
Do not promote a repo to `enforce` on the user's behalf; this surface only reports. Recommend the flip only when every candidate rule reads `safe_to_enforce` and the window is not truncated.
