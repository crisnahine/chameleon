...
- **Never auto-demote.** This surface only reports. Demotion is a human decision routed through refresh-time recalibration; nothing here mutates `enforcement.json` at runtime (that artifact is in the trust hash, so a runtime flip would de-trust the profile and silently disable blocking repo-wide).
