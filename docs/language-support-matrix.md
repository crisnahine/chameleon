...
| Doctor advisory-emission health check (source-edit attribution) | ✅ | ✅ | ✅ | Python: full. .py/.pyi are in the doctor `_source_exts` set (doctor.py), so a Python repo where archetype resolution silently stops firing triggers the 'advisories not firing' diagnostic, like TS/Ruby. Full parity. |

### 12. Framework awareness

| Capability | TS | Ruby | Py | Notes |
|---|:--:|:--:|:--:|---|
| Framework role/archetype detection | ✅ | ✅ | ✅ | Implemented. `_TS_PRIORS` covers Next.js (app + pages router), Remix routes, and NestJS/Angular filename-role suffixes. The suffix set is the roles a real Nest codebase carries: `*.controller.ts` / `*.service.ts` / `*.module.ts` / `*.resolver.ts` / `*.gateway.ts` / `*.guard.ts` / `*.dto.ts` / `*.entity.ts` / `*.repository.ts` / `*.interceptor.ts` / `*.filter.ts` / `*.decorator.ts` / `*.pipe.ts` / `*.middleware.ts` / `*.strategy.ts` (`.config.ts` is deliberately excluded -- shared with Next.js/Vite/Jest, and the config-module prior already covers it). A `route.ts` and a util no longer get the same treatment. Python's role table likewise covers Django's built-ins plus the service layer real codebases add (`services.py` / `selectors.py` / `repositories.py` / `exceptions.py` / `mixins.py` / `factories.py` / `policies.py` / `clients.py`; `base`/`utils`/`helpers` excluded as grab-bags). A cohort in an unrecognized directory takes its name from that directory (singularized) rather than a `cluster-<hash>`. Full parity with the Ruby Rails priors. |
...
- **TypeScript** - the extractor now scrubs `NODE_OPTIONS` / `NODE_REPL_EXTERNAL_MODULE` (matching the Ruby/Python interpreter-option scrubs); per-method `decorators`, a qualified `enclosing_class_path` (namespace + class), and `base_class` are attached to callable signatures; the `_TS_PRIORS` table gained the full NestJS/Angular filename-role suffix set (`*.controller.ts` / `*.service.ts` / `*.module.ts` / `*.guard.ts` / `*.resolver.ts` / `*.gateway.ts` / `*.dto.ts` / `*.entity.ts` / `*.repository.ts` / `*.interceptor.ts` / `*.filter.ts` / `*.decorator.ts` / `*.pipe.ts` / `*.middleware.ts` / `*.strategy.ts`), so a real Nest repo's `.dto`/`.entity`/`.repository` cohorts (often the largest) become named role archetypes instead of per-feature `cluster-<hash>` buckets.
