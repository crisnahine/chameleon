...
**Block-eligible rules.** `violation_class.BLOCK_ELIGIBLE_RULES`: the archetype-independent `phantom-import`, `secret-detected-in-content`, and `eval-call`; and the archetype-dependent `import-preference-violation`, `jsx-presence-mismatch`, `naming-convention-violation`, `inheritance-convention-violation`, and `file-naming-convention-violation`. The dependent rules block only at `confidence=high` + `match_quality=ast` so a wrong archetype match cannot make them spurious. Naming and inheritance violations are always `warning` severity; only `jsx-presence-mismatch` is severity-gated (qualifies at `error` only). `secret-detected-in-content` is the deliberate promotion of secrets from advisory to block: it is kind-gated to the high-precision deterministic patterns only (AKIA, ghp_, sk-ant-, sk_live, PEM, plus Google AIza, GCP service-account, Azure AccountKey), and `scan_secrets` is now wired into the in-process lint path so the hook actually evaluates it. JWT `eyJ` and `://user:password@` stay advisory: both match benign committed content (base64 JSON, docker-compose test configs) and calibration cannot measure their precision because clean repos never trip them. `eval-call` is the one dangerous sink promoted to block (verified zero hits across the real test repos, so calibration keeps it active); `exec-call` and `raw-sql-concat` stay advisory because `.exec()` is a common JS string method and the SQL regex matches the words SELECT/DELETE inside any interpolated template.
...
- **Stage 2 - enforce, monitor overrides.** Flip `enforcement.mode` to `enforce`. Block-eligible rules now deny/block for real, gated by per-repo calibration so a rule that flags the repo's own committed files stays advisory. The lead watches the override-rate panel (`get_override_audit`): a rule overridden in a large fraction of edits is fighting the team, so either the convention is wrong (refresh/teach) or the rule is miscalibrated (recalibrate at refresh, never a runtime mutation). Blanket bare ignores are flagged separately. Gate to the next stage: override rate stable and low, no blanket-ignore abuse.
...
0. **Block-eligible deterministic secret detection** - `secret-detected-in-content` is promoted from advisory to block, kind-gated to high-precision deterministic patterns only (AKIA, ghp_, sk-ant-, sk_live, PEM, Google AIza, GCP service-account, Azure AccountKey). Wired into the in-process lint path, archetype-independent, calibrated. PR-review escalates the same rule to a BLOCK verdict. JWT/`user:password@` stay advisory (benign-content collisions, unmeasurable precision).
