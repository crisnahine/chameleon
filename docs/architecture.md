...
| Point | Hook | Trigger |
|-------|------|---------|
| PreToolUse deny | `preflight-and-advise` | A banned/competing import in the *proposed* content (`import-preference-violation`) — denied before the write runs. Also a deterministic hard-kind secret in the proposed content (`secret-detected-in-content`): archetype-independent (it fires even when the file resolves no archetype and regardless of match quality/confidence), gated on a trusted profile + the rule being calibration-active + enforce mode, scanning the first 100KB of the proposed content (`PREWRITE_SECRET_SCAN_MAX_CHARS`) via the regex-only hard-kind scanner — never detect-secrets — so the hot-path budget holds on multi-MB payloads. |
...
Shadow is a faithful preview of enforce, not a violations census: a PostToolUse `would_block` row is recorded only under the same conditions the real block needs (file at L2 escalation, high-confidence AST match), so a single-fix advisory edit adds an advisory row, never a would-block row. The PreToolUse deny gates have no escalation prerequisite, so their shadow counters don't either — the secret deny shares the import deny's discipline here, and additionally emits its `would_block` rows in enforce mode too (capped at 3 per edit), so a real deny stays visible to `/chameleon-explain` and the shadow report's rule history. In the shadow report, `total_edits` counts completed verified edits — an edit denied at PreToolUse never completes, so a deny-only session truthfully reads `total_edits=0` alongside populated per-rule rows.
...
**Escape hatch and kill switch.** Any block is overridable inline with `// chameleon-ignore <rule>` (`# chameleon-ignore <rule>` in Ruby), or a bare `// chameleon-ignore` to suppress all chameleon blocks on that line — except the deterministic-kind hard class (hard-kind secrets, error-severity eval-call), which the bare form never covers: a runnable security fact is never one blanket comment away from silence, so suppressing one requires the rule name (`// chameleon-ignore secret-detected-in-content`, `// chameleon-ignore eval-call`), keeping the bypass deliberate and auditable. The directive must end its line (only whitespace and a block-comment closer may follow the rule name), so prose that merely mentions a directive never activates one; string-literal bodies are blanked before the scan, so attacker-controllable content (a help string quoting the directive) cannot switch a rule off. Scope: a trailing directive covers its own line; a directive on a line of its own also covers the line below; `// chameleon-ignore-file <rule>` covers the whole file. Violations that report a line number (deterministic secrets, eval-call) honor that scoping strictly; file-level violations with no line (naming/inheritance/structure) accept a named directive anywhere in the file, since there is no line to target. For the pre-write secret deny on Edit fragments, a NAMED file-scope directive in the on-disk target file is honored too (read lazily, only after a hard hit), so a fixture file annotated once does not deny every later fragment edit. `CHAMELEON_ENFORCE=0` forces advisory-only for the whole session regardless of mode. Every gate fails open: a missing/corrupt config or calibration artifact degrades to advisory, never an erroneous block.
...
.chameleon/ (committed, team-shared, atomic-write-protected)
 ...
 ├── calls_index.json # callee -> deterministic caller rows (TS+Ruby) - backs judge caller facts
...
### Symbol and catalog index artifacts

Four committed artifacts make the cross-file checks possible without re-parsing callers on the hot path. The two symbol indexes are TypeScript-only (Prism emits no export names; Ruby method resolution is dynamic, a guaranteed FP firehose under Rails metaprogramming); the function catalog and the calls index cover both supported languages, because they assert only per-file declaration facts and statically unambiguous call edges.
...
- **`calls_index.json`** - the inverse CALL view (TS + Ruby): callee file -> callable name -> recorded caller rows (caller file, enclosing function, line, grade), built from the dumpers' `call_sites` at bootstrap/refresh. Entries carry an honest `total` and a `truncated` flag; the per-callee cap, the global edge cap, and a dump-time site cap all mark the count a lower bound instead of silently undercounting. Unlike the other index artifacts, a failed rebuild DROPS the old copy rather than carrying it forward: stale caller facts fed to the judge are worse than none.

### Calls index and judge caller facts

The calls index stores exactly three deterministic grades; name-only repo-wide matches are never stored because they are the false-positive bulk.

- `same_file` - a bare call to a callable defined in the caller's own file, or a `this.`/`self.` call to a member of a class defined in the same file. Cross-file inheritance is out of scope: a this-call whose method lives on a base class elsewhere yields no edge rather than a guess.
- `import` (TS only) - a bare or `new` call of a named import, matched on its LOCAL binding name (`import { x as y }` binds `y`) and recorded under the exported name it resolves to, or a `ns.member()` / `new ns.Foo()` through a runtime namespace import. The member form is a single property hop only: `ns.utils.helper()` dispatches through a property the export set proves nothing about, so multi-hop chains record no receiver and never edge. The callee must exist in the target's CLOSED export set; a barrel (`export * from`) proves nothing and yields no edge.
- `constant_receiver` (Ruby only) - `Const.method` where `Const` matches a class key exactly. Keys are fully qualified (`enclosing_class_path`, module nesting included; old dumps fall back to the lexical class name), so a bare receiver matches a top-level class only; a bare name CAN lexically resolve to a namespaced class from inside its namespace, but call sites carry no lexical context, so that edge stays unasserted (accepted undercoverage). The matched key must name exactly one defining file AND the matched member must be class-level (`def self.x` or a `class << self` def, dump kind `singleton_method`); an instance def with the same name is undispatchable from a constant receiver. `Const.new` maps to the instance `initialize`, is skipped when the target does not define one, and is skipped when the class overrides `def self.new` (the override owns construction; mapping through it is unprovable).

Two imprecision classes are accepted as known limitations, alongside the Rails-metaprogramming exclusion above. Binding shadowing: a parameter or local variable that shadows an imported binding can yield a false `import` edge, because call-site identifiers are matched against the import map with no scope analysis, by design. Barrel re-export attribution: a named re-export barrel (`export { x } from './impl'`) has a closed export set, so edges attribute to the barrel module rather than the file that defines the implementation (only wildcard `export * from` barrels are open sets that yield no edge at all).

The primary consumer is the turn-end correctness judge: `caller_facts_for_diffs` renders a bounded facts block listing, for each callable the turn actually changed, its committed callers (or an explicit "no committed callers found" line that allows new/unused/dynamic call paths; absence of an edge is never evidence of dead code). The block is toggled by `enforcement.judge_crossfile_facts` (default on), and each judge run records exactly one `judge_facts_*` check event (`judge_facts_included`, `judge_facts_skipped_disabled`, or `judge_facts_skipped_no_calls_index`) so a post-incident replay can tell whether the reviewer actually saw the facts. The same artifact backs the `get_callers` MCP tool (on-demand caller blast radius for one function, trust-gated like the other read tools) and the pr-review skill's caller listing. Reads fail open everywhere: a missing or corrupt artifact means no facts block, never a crash or a fabricated claim.

### Trust hashing

`/chameleon-trust` records a SHA-256 over the user-visible profile surface so a post-trust change re-prompts. The hash (`hash_profile` in `profile/trust.py`) covers, in alphabetical filename order with per-file framing: `.archetype_renames.json`, `archetypes.json`, `calls_index.json`, `canonicals.json`, `config.json`, `conventions.json`, `enforcement.json`, `exports_index.json`, `function_catalog.json`, `idioms.md`, `principles.md`, `profile.json`, `reverse_index.json`, and `rules.json`. The new convention, index, and catalog artifacts are all in the set, so a refresh that changes any of them de-trusts the profile (flips it to `stale`) until the user re-approves. `enforcement.json` being in the hash is deliberate: it is why the override audit never auto-mutates the calibration verdict at runtime (that would silently flip the profile to stale and disable all blocking repo-wide); a recalibration runs only at refresh time, which legitimately rewrites the verdict before the next hash snapshot. Only the prose artifacts that a hostile committer could weaponize for prompt injection (`idioms.md`, `principles.md`) are content-scanned at trust time; `conventions.json` / `canonicals.json` carry real code shapes and are not.
