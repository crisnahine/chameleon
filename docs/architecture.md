...
- `import` (TS only) - a bare or `new` call of a named import, matched on its LOCAL binding name (`import { x as y }` binds `y`) and recorded under the exported name it resolves to, or a `ns.member()` / `new ns.Foo()` through a runtime namespace import. The callee must exist in the target's CLOSED export set; a barrel (`export * from`) proves nothing and yields no edge.
- `constant_receiver` (Ruby only) - `Const.method` where `Const` names exactly one class across the dump AND the matched member is class-level (`def self.x` or a `class << self` def, dump kind `singleton_method`); an instance def with the same name is undispatchable from a constant receiver. `Const.new` maps to the instance `initialize`, and is skipped when the target does not define one.
